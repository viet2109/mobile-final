...
const SelectPurposeScreen: React.FC = () => {
  ...
  const { sender } = useLocalSearchParams<Params>();
  const senderData: Sender = JSON.parse(sender || "{}");

  const purposes: Purpose[] = [
    {
      name: "Personal",
      des: "Receive money from friends or family",
      icon: (
        <FontAwesome
          name="user-o"
          size={22}
    ...
    {
      name: "Business",
      des: "Receive payments from clients",
      icon: (
        <Ionicons
          name="bag-handle"
          size={22}
